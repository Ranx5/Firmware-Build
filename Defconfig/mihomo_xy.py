...
with open('./config.yaml', 'rb') as f:
    x = yaml.safe_load(f)
Proxy = {'HK':[], 'CA':[],'JP':[], 'TW':[], 'UK':[], 'US':[], 'OT':[], 'All':[]}
testtime='300'
n = len(Proxy)
ProxySet = set()
pgs = []
...
for s in Strategy3:
    pgs.append({'name':s, 'type':'select', 'proxies':ProxySet+Proxy['All']+['DIRECT']})
for s in ProxySet:
    if s != 'OT':
...
if os.path.exists('/etc/nikki/profiles/config_xy.yaml'):
    os.system('rm /etc/nikki/profiles/config_xy.yaml')
    print('删除旧配置！')
with open('/etc/nikki/profiles/config_xy.yaml', 'w') as file:
